...
func (d *DB) ListPosts(category string) ([]*Post, error) {
    var rows *sql.Rows
    var err error
    base := `SELECT p.id, p.title, p.content, p.category, u.username, p.created_at, COALESCE((SELECT SUM(value) FROM post_votes v WHERE v.post_id = p.id),0) FROM posts p JOIN users u ON u.id = p.author_id`
    if category == "" {
    ...
    for rows.Next() {
        p := &Post{}
        var created int64
        if err := rows.Scan(&p.ID, &p.Title, &p.Content, &p.Category, &p.Author, &created, &p.Score); err != nil {
            return nil, err
        }
...
func (d *DB) GetPost(id int) (*Post, error) {
    row := d.db.QueryRow(`SELECT p.id, p.title, p.content, p.category, u.username, p.created_at, COALESCE((SELECT SUM(value) FROM post_votes v WHERE v.post_id = p.id),0) FROM posts p JOIN users u ON u.id = p.author_id WHERE p.id = ?`, id)
    p := &Post{}
    var created int64
    if err := row.Scan(&p.ID, &p.Title, &p.Content, &p.Category, &p.Author, &created, &p.Score); err != nil {
        return nil, err
    }
    p.CreatedAt = time.Unix(created, 0)
    return p, nil
...
func (d *DB) ListComments(postID int) ([]*Comment, error) {
    rows, err := d.db.Query(`SELECT c.id, c.post_id, c.content, u.username, c.created_at, COALESCE((SELECT SUM(value) FROM comment_votes v WHERE v.comment_id = c.id),0) FROM comments c JOIN users u ON u.id = c.author_id WHERE c.post_id = ? ORDER BY c.created_at`, postID)
    if err != nil {
        return nil, err
    }
    defer rows.Close()
    ...
    for rows.Next() {
        c := &Comment{}
        var created int64
        if err := rows.Scan(&c.ID, &c.PostID, &c.Content, &c.Author, &created, &c.Score); err != nil {
            return nil, err
        }
